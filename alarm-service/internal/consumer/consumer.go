...
// TelemetryMessage matches telemetry payload format
...
type MessageHandler func(ctx context.Context, msg *TelemetryMessage) error
...
func (c *Consumer) Start(ctx context.Context) error {
	c.logger.Info("Starting Kafka consumer for alarm-service",
		...
		go func(id int, r *kafka.Reader) {
			...
			for {
				...
				default:
					...
					if err := c.handler(ctx, &telMsg); err != nil {
						c.logger.Error("Failed to process message in alarm-service",
							zap.Int("worker_id", id),
							zap.Error(err),
							zap.String("machine_id", telMsg.MachineID),
							zap.String("metric", telMsg.Metric),
						)
					}
